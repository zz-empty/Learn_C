...
**12.1 处理文件中的非小写字母**
    ...
    - 思路：
        1. 循环获取文件中的每一行
        2. 遍历每一行中的字符，挨个处理判断，然后将新的字符串存储到一个新的文件中

**12.2 统计文件中词频并打印词频前10的信息**

    - 思路：
        1. 维护一个小根堆，堆顶是词频最小的单词
        2. 使用哈希表，记录每个单词的词频，键是单词，值是词频
        2. 遍历每个单词，更新小根堆

**12.3 有一个学生信息文件，每一行记录了一个学生信息，格式（学号\t 姓名\t 性别\t 分数 1\t 分数 2\t 分数 3\n）。
     要求：读取信息放入链表，按总分递减的顺序保存回源文件**

     - 思路：设计一个链表结构，存储所有信息，按总分排序，再写回文件
